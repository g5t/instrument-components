...
from scipp import Variable, vector, scalar, array
...
class Analyzer:
    ...
    def from_calibration(position: Variable, focus: Variable, **params):
        ...
        shape = sum(vector(x, unit='1') * s for x, s in (([1, 0, 0], width), ([0, 1, 0], length), ([0, 0, 1], depth)))
...
class Channel:
    ...
    def from_calibration(relative_angle: Variable, **params):
        ...
        known_d_lengths = {
            'short': array(values=[217.9, 242.0, 260.8, 279.2, 298.8], unit='mm', dims=['analyzer']),
            'symmetric': array(values=[226.0, 249.0, 267.9, 286.3, 304.8], unit='mm', dims=['analyzer']),
            'long': array(values=[233.9, 255.9, 274.9, 293.4, 311.9], unit='mm', dims=['analyzer']),
        }
        known_a_lengths = {
            'short': array(values=[134, 147.1, 156.2, 165.2, 175.6], unit='mm', dims=['analyzer']),
            'symmetric': array(values=[142, 154.1, 163.2, 172.3, 181.6], unit='mm', dims=['analyzer']),
            'long': array(values=[149.9, 161, 170.2, 179.3, 188.6], unit='mm', dims=['analyzer']),
        }
        known_a_widths = {
        ...
        ks = sqrt(energies * 2 * neutron_mass) / hbar
        ...
        coverages = tan(min(ks) * atan(coverage) / ks)

        analyzers = []
        detectors = []
        for ap, dp, ct, cl, cw, cd, cc in zip(analyzer_positions, detector_positions, counts, crystal_lengths, crystal_widths, crystal_depths, coverages):
